...
class Boggle():
    ...
    def find_from(self, board, word, y, x, seen):
        ...
        # Recursively to find smaller and smaller words until all tries are exhausted or until success.

        # This isn't the letter we're looking for.
        if board[y][x] != word[0]:
            return False

        # We've used this letter before in this current path
        if (y, x) in seen:
            return False

        # Down to the last letter --- so we win!
        if len(word) == 1:
            return True
